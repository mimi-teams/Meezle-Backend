...
public class EventApi extends BaseGenericApi<EventService> {
    private final Logger                  logger = LogManager.getLogger(EventApi.class.getName());
    ...
    @Operation(method = "GET", summary = "이벤트 정보 반환",
    ...
    @Operation(method = "POST", summary = "새로운 이벤트 등록", description = "[로그인 O, 인가 O] 새로운 이벤트를 등록한다. 가입한 이용자만 이용할 수 있다",
               responses = {@ApiResponse(useReturnTypeSchema = true)})
    @PostMapping(path = "")
    public @Valid ApiCallResponse<EventResponseDto> post(@Valid
                                                         @RequestBody
    ...
    @Operation(method = "PATCH", summary = "이벤트 정보 수정",
    ...
    @Operation(method = "DELETE", summary = "이벤트 삭제",
               description = "[로그인 O, 인가 O] ID에 해당하는 이벤트를 삭제한다. 이벤트 생성자만 가능하며, 삭제 시 Event 와 연관된 정보가 모두 삭제된다. 삭제 후, " +
                             "'/'로 Redirect 된다", responses = {@ApiResponse(description = "'/'로 Redirect", content = {
            @Content(schema = @Schema(description = "GET '/'"))})})
    ...
    @Operation(method = "GET", summary = "이벤트 생성자 가져오기",
    ...
    @Operation(method = "GET", summary = "모든 참여자 가져오기",
    ...
    @Operation(method = "GET", summary = "참여자 가져오기",
    ...
    @Operation(method = "POST", summary = "이벤트에 참여자 등록하기", description = "[로그인 O, 인가 O] 이벤트에 참여자를 등록시칸다. 이벤트 생성자만 가능하다",
               responses = {@ApiResponse(useReturnTypeSchema = true)})
    ...
    public @Valid ApiCallResponse<EventParticipantResponseDto> joinEvent(
            ...
            Long id, @Valid
            @RequestBody
            EventParticipantRequestDto requestDto) {
        final var loginInfo = authService.getLoginInfo(httpSession);
        authService.isHost(loginInfo, id);
    ...
    @Operation(method = "PATCH", summary = "이벤트 참여자 정보 수정하기",
    ...
    @Operation(method = "PATCH", summary = "이벤트 참가 시간 설정하기",
    ...
    public @Valid ApiCallResponse<EventResponseDto> calculateTime(
            ...
            @NotNull @Pattern(regexp = "^(calculate|modify)$") @Valid
            @RequestParam
            String mode,
            @RequestBody
            @Nullable
            @Valid EventParticipantRequestDto requestDto) {
        final var loginInfo = authService.getLoginInfo(httpSession);
        authService.isHost(loginInfo, id);
        if(Objects.equals(mode, "calculate")) {
            ...
            if(Objects.nonNull(requestDto)) {
                final var event = service.modifySelectedDaysAndTimesDirectly(requestDto);
                return ApiCallResponse.ofSuccess(EventResponseDto.of(event));
            } else {
