...
public class UserApi extends BaseGenericApi<UserService> {
    ...
    @Operation(method = "DELETE", summary = "회원 탈퇴",
    ...
    @Operation(method = "GET", summary = "플랫폼을 이용한 로그인",
    ...
    @Operation(method = "GET", summary = "가입한 이용자의 로그아웃",
               description = "[로그인 O, 인가 X] 이용자의 로그아웃을 처리한다. 로그아웃 후, '/'로 Redirect 된다", responses = {
            @ApiResponse(description = "'/'로 Redirect",
                         content = {@Content(schema = @Schema(description = "GET '/'"))})})
