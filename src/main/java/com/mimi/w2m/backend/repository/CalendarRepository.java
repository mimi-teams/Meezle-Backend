...
public interface CalendarRepository extends JpaRepository<Calendar, UUID> {
    @Query(value = "select calendar from Calendar calendar where calendar.user = :user and calendar.event = :event")
    Optional<Calendar> findByUserAndEvent(@Param("user") User user, @Param("event") Event event);
